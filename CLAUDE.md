...
| 6b · Progress you can see | Every long step (extraction, generation, judging, scoring, regeneration, pre-scoring, sample load) shows a live, honest progress state: what is happening now, k of n, elapsed and estimated remaining, actual cost so far, the last variant that finished, and a clear failed/partial state with a Resume button. No blank screens, no spinner without words | Import, Generate, Report, Grade |
| 6c · Knobs from the paper | Expose the parameters the paper treats as design choices, with defaults from the pilot: thresholds per property, joint-score weights, diversity metric options, judge design (model, samples, aggregation, scale), decoding and concurrency, strategy-specific settings (anchors, construct map, surface dimensions, readability band), outlier and regeneration policy, release policy. Institution-level knobs live on the console and are versioned; per-run knobs on Generate; the rest under an Advanced panel. Inventory produced by the wave 6c analysis | thresholds, Generate, Console, Settings |
| 6d · Cost | Prompt caching on the shared blueprint/rubric/model-answer prefix (cache reads at a tenth of input rate); two presets on Generate: "High-stakes" (Opus 5, 5 judge samples) and "Formative" (Sonnet 5, 3 judge samples); show per-student cost on the estimate | llm, Generate |
| 9 · Guided journey | Non-technical instructor first: a StepIntro narrator on every step (what it is, what to do, what happens next), one primary action per page with everything else behind "Show options", plain words first with `Info` buttons on every term and a `/glossary`, verdicts as sentences before numbers, the rail showing done/current/next with plain step names (six steps, 1-based) | shell, all instructor pages, glossary |
| 10 · Responsive | Renders from 360px to 1440px: rail becomes a top bar with a drawer below 900px, grids collapse via `.va-two/.va-tiles/.va-split/.va-path-grid`, tables scroll in `.va-table-scroll`, SVGs scale, 44px touch targets; `scripts/qa-responsive.mjs` | shell, app.css, all pages |
| 11 · Controlled numbers | Numbers in an assignment are variables the instructor controls (keep / vary within a range / derived by formula), chosen per version by seeded code, written into the prompt as the only figures allowed, checked for consistency in the version and its model answer (regenerate once on failure), judged for solvability, and summarised on the report with a numeric-difficulty spread. UI: "Numbers in this assignment" on Check what we found, a switch on Make the versions, figures and a stamp on the report | `src/lib/quantities`, llm prompts, orchestrator, Blueprint/Generate/Report |
